...
## 2026-08-20 — LK-005
Task: Eval harness
- Added `api/evals/lookup.eval.js`: a standalone pre-deploy harness (not `node:test` — calls REAL LLM backends) that runs `performLookup()` from `api/src/lookup.js` against a golden case set and checks: structural invariants (≤4 blocks, ≤8 groups, ≤10 cards/group, no near-duplicate card text, empty groups already dropped) independently of the request-time validator; reading-token correctness against a small hardcoded golden set of known words/readings (`GOLDEN_READINGS`); and a conversational-quality LLM-judge pass (`"would a person actually say this to someone they want to connect with?"`, JSON verdict, 70% pass-rate threshold) on a sample of produced cards.
- Golden-set sampling strategy (documented in the file's header comment): the full input space is 4 languages × 4 abilities × 3 formalities × 4 audiences × 3 llms = 576 combinations; `GOLDEN_CASES` samples a representative subset instead — 6 anchor terms (the 3 API_DESIGN.md worked examples + 3 more covering the remaining languages/backends) with every axis value assigned round-robin, so every axis value appears at least once without running the full cross-product.
- Runnable via `npm run eval:lookup` (from `api/src/`) or directly `node api/evals/lookup.eval.js`; supports `--limit=N`, `--llm=<name>`, `--no-judge` for cheaper partial runs. Prints a pass/fail report and sets `process.exitCode`.
- Files changed: `api/evals/lookup.eval.js` (new), `api/src/package.json` (`eval:lookup` script).
- **Live-verified**: ran `npm run eval:lookup -- --limit=2 --llm=claude` against the real `claude` backend — structural checks passed, the golden reading for `菜单` matched exactly, and the LLM-judge correctly flagged one stiff card ("我要看菜单。") while the run still passed overall (3/4 ≥ 70% threshold). Confirms every check path (schema, structural, golden reading, judge) actually exercises, not just parses.
- **Learnings:** the eval harness reuses `performLookup` (the pure core extracted in LK-004) rather than re-implementing the request flow — any future finish-layer change is exercised by both the unit tests and the eval harness for free. Keep that core pure (no req/res) so it stays reusable this way.