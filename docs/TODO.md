...
- [x] **Web: deck title tap menu ("Settings" / "Edit cards")** — DESIGN.md specifies tapping the deck title in the header reveals a two-item menu: "Settings" (slides up deck-settings action pane) and "Edit cards" (enters reorder mode). Not present in current web implementation.
- [x] **iOS: deck title tap menu** — Same as above; not present in current iOS implementation.
...
- [x] **iOS: Generate Cards action pane (bottom sheet)** — Equivalent of the above web Generate Cards sheet. Wire to `POST /generate-cards`. On success, add cards to deck and close. Auto-open for new/empty decks with language selector.
