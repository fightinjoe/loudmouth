...
import { getOrCreateAllDeck, createDeck, getDecks, getCards, importCards } from '../db.js';

let store;

beforeEach(async () => {
  const idb = new IDBFactory();
  store = new Dexie('loudmouth', { indexedDB: idb, IDBKeyRange });
...
describe('getCards', () => {
  it('returns only cards belonging to the given deck', async () => {
    ...
    await importCards([{ lang: 'zh', type: 'word', front: { text: 'A' }, back: { translation: 'a' } }], deck1.id, store);
    await importCards([{ lang: 'zh', type: 'word', front: { text: 'B' }, back: { translation: 'b' } }], deck2.id, store);

    const cards = await getCards(deck1.id, store);
    expect(cards).toHaveLength(1);
    expect(cards[0].front.text).toBe('A');
  });

  it('returns all-deck cards', async () => {
    await importCards([{ lang: 'zh', type: 'word', front: { text: 'A' }, back: { translation: 'a' } }], null, store);
