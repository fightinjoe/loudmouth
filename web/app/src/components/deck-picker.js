...
export async function openDeckPicker(
  appEl,
  { db, getDecks, getRecentDecks, getCardsByLang },
  onSelectDeck,
  onAddCards,
) {
  ...
  if (getCardsByLang) {
    ...
    for (const lang of Object.keys(byLang)) {
      ...
      starredCounts[lang] = allCards.filter(
        (c) => c.lang === lang && c.state?.starredAt,
      ).length;
    }
  }
  ...
  panel.querySelector(".deck-list").addEventListener("click", (e) => {
